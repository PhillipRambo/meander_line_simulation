...
def generate_meander_antenna(lambda_g, d, s, L, w, N=4, num_points=200, plot=True):
    """
    Generate a meander line antenna structure, a centerline, and discretize it.
    
    Parameters:
    - lambda_g: wavelength in the substrate
    - d: vertical wire segment length
    - s: horizontal wire  segment length
    - L: overall antenna length for plotting
    - w: width of  wire segments
    - N: number of horizontal wire segments
    - num_points: number of points along centerline
    - plot: bool, whether to plot the antenna
    
    Returns:
    - discrete_points: array of shape (num_points, 2) with (x, y) coordinates
    """
