...
def compute_fields_for_points(observer_points, points_3d, eta, k, I0):
    ...
    for obs in observer_points:
        ...
        fields_complex = compute_electrical_and_magnetic_field(
            eta, k, I0, h_list, r_array, phi_hat, r_hat, theta_hat, sin_theta, cos_theta, s_hat_array, magnitude=True
        )
        ...
        E_total_complex.append(fields_complex['E_total_spherical'])
        H_total_complex.append(fields_complex['H_total_spherical'])
                              
    E_total_array = np.array(E_total)
    H_total_array = np.array(H_total)
    E_total_array_complex = np.array(E_total_complex)
    H_total_array_complex = np.array(H_total_array_complex)


    return E_total_array, H_total_array, E_total_array_complex, H_total_array_complex
...
def calculate_directivity(E_field, H_field, observationpoints):
    ...
    Prad = ( ( 8 * np.pi ) / ( dphi * dtheta ) ) * ( 1 /np.real(np.sum(np.cross(E_field,np.conj(H_field)))) * np.sin(theta) )
    S_max = np.max(np.cross(E_field,np.conj(H_field)))
    D0 = S_max / Prad
